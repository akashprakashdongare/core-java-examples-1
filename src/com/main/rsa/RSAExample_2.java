...
public class RSAExample_2 {
	public static void main(String[] args) {
		...
		String encrypted_str = getEncrypted(data, publicKey);
		System.out.println(" Encrypted : " + encrypted_str);
		System.out.println(" Decrypted : " + getDecrypted(encrypted_str, privateKey));
	...
	private static String getDecrypted(String data, String key) {
		...
		try {
			KeyFactory keyFactory = KeyFactory.getInstance("RSA");
			// PrivateKey private_key = keyFactory.generatePrivate(new
			// PKCS1EncodedKeySpec(Base64.getDecoder().decode(key)));
			...
			byte[] bytes = new byte[pkcs8Header.length + pkcs1Bytes.length];
			System.arraycopy(pkcs8Header, 0, bytes, 0, pkcs8Header.length);
			System.arraycopy(pkcs1Bytes, 0, bytes, pkcs8Header.length, pkcs1Bytes.length);
